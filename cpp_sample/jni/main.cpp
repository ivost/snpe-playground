...
Processing DNN Input: /home/ivo/github/snpe-playground/models/ssd_mobilenet_v2_coco/data/cropped/bear.raw
Save path /tmp/Result_0/detection_classes:0.raw

 */
int main(int argc, char **argv) {
    enum {
        UNKNOWN, USERBUFFER_FLOAT, USERBUFFER_TF8, ITENSOR
    };
    enum {
        CPUBUFFER, GLBUFFER
    };
    ...
    static std::string dlc = "/home/ivo/github/snpe-playground/models/ssd_mobilenet_v2_coco/dlc/mobilenet_ssd.dlc";
    static std::string OutputDir = "/tmp/output/";
    const char *inputFile = "/home/ivo/github/snpe-playground/models/ssd_mobilenet_v2_coco/data/cropped/file_list.txt";
        ...
        switch (opt) {
            case 'h':
                std::cout
                        << "\nDESCRIPTION:\n"
                        << "------------\n"
                        << "Load and execute a neural network\n"
                        ...
                        << "OPTIONAL ARGUMENTS:\n"
                        << "-------------------\n"
                        << "  -b  <TYPE>   Type of buffers to use [USERBUFFER_FLOAT, USERBUFFER_TF8, ITENSOR] ("
            ...
            case 'c':
                ...
                break;
            default:
                std::cout << "Invalid parameter specified. Please run me with the -h flag to see required arguments"
                          << std::endl;
                std::exit(FAILURE);
        }
    }
    ...
    if (!dlcFile) {
        std::cout << "dlc file not found: " << dlc << std::endl;
        return EXIT_FAILURE;
    }
 ...
 */
    ...
    if (useUserSuppliedBuffers) {
        ...
        if (bufferType == USERBUFFER_TF8) {
            ...
            if (userBufferSourceType == CPUBUFFER) {
                ...
                for (size_t i = 0; i < inputs.size(); i++) {
                    ...
                    // Save the execution results only if successful
                    if (execStatus == true) {
                        if (!saveOutput(outputMap, applicationOutputBuffers, OutputDir, i * batchSize, batchSize,
                                        false)) {
                            return EXIT_FAILURE;
                        }
                    } else {
        ...
        for (size_t i = 0; i < inputs.size(); i++) {
            ...
            if (execStatus == true) {
                cout << "outputTensorMap size " << outputTensorMap.size() << endl;
