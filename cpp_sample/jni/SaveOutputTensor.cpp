...
bool saveOutput(zdl::DlSystem::TensorMap outputTensorMap,
    ...
    for (auto &name : tensorNames) {
        ...
        for (size_t i = 0; i < batchSize; i++) {
            ...
            std::cout << "Save path " << path.str() << std::endl;
    ...
    for (auto &name : outputBufferNames) {
        for (size_t i = 0; i < batchSize; i++) {
            ...
            if (isTf8Buffer) {
                ...
                zdl::DlSystem::UserBufferEncodingTfN ubetf8 = dynamic_cast<zdl::DlSystem::UserBufferEncodingTfN &>(outputMap.getUserBuffer(
                        name)->getEncoding());
                output.resize(applicationOutputBuffers.at(name).size() * sizeof(float));
                Tf8ToFloat(reinterpret_cast<float *>(&output[0]), applicationOutputBuffers.at(name).data(),
                           ubetf8.getStepExactly0(), ubetf8.getQuantizedStepSize(),
                           applicationOutputBuffers.at(name).size());
